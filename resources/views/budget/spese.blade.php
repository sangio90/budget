                        ...
                        <tr class="hover:bg-slate-50 transition">
                            ...
                            <td class="px-5 py-3 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <form method="POST" action="{{ route('budget.destroy', $s) }}" onsubmit="return confirm('Eliminare questa spesa?')">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="text-slate-300 hover:text-red-400 transition" title="Elimina">
                ...
                <div class="px-4 py-3">
                    <div class="flex items-start justify-between gap-2">
                        ...
                        <div class="flex items-center gap-2 flex-shrink-0">
                            <span class="font-semibold text-sm text-red-500">{{ number_format($s->importo, 2, ',', '.') }} €</span>
                            <a href="{{ route('budget.edit', $s) }}" class="text-slate-300 hover:text-indigo-500 transition">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            </a>
                            <form method="POST" action="{{ route('budget.destroy', $s) }}" onsubmit="return confirm('Eliminare?')">
                                @csrf @method('DELETE')
